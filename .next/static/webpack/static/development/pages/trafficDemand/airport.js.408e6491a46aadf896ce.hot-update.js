...
function (_Component) {
  ...
  function DashboardLayout() {
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "handleSelectOption", function (e, data) {
      ...
      if (data.name === 'routeB') {
        var _status = !_this.state.routeB;

        _this.setState(_defineProperty({}, data.name, _status));

        _this.forceUpdate();
      }

      if (data.name === 'airlineB') {
        var _status2 = !_this.state.airlineB;

        _this.setState(_defineProperty({}, data.name, _status2));

        _this.forceUpdate();
      }

      if (data.name === 'aircraftB') {
        var _status3 = !_this.state.aircraftB;

        _this.setState(_defineProperty({}, data.name, _status3));

        _this.forceUpdate();
      }

      if (data.name === 'seatB') {
        var _status4 = !_this.state.seatB;

        _this.setState(_defineProperty({}, data.name, _status4));

        _this.forceUpdate();
      }

      if (data.name === 'frequencyB') {
        var _status5 = !_this.state.frequencyB;

        _this.setState(_defineProperty({}, data.name, _status5));

        _this.forceUpdate();
      }

      if (data.name === 'paxB') {
        var _status6 = !_this.state.paxB;

        _this.setState(_defineProperty({}, data.name, _status6));

        _this.forceUpdate();
      }

      if (data.name === 'loadfactorB') {
        var _status7 = !_this.state.loadfactorB;

        _this.setState(_defineProperty({}, data.name, _status7));
...
//# sourceMappingURL=airport.js.408e6491a46aadf896ce.hot-update.js.map